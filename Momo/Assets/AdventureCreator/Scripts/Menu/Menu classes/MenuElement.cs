...
namespace AC
{
	...
	public class MenuElement : ScriptableObject
	{
		...
		protected string TranslateLabel (string label, int languageNumber)
		{
			...
			return KickStarter.runtimeLanguages.GetTranslation (label, lineID, languageNumber, AC_TextType.MenuElement);
		}
		...
		protected void SetAbsoluteSize (Vector2 _size)
		{
			...
			slotSize = new Vector2 (_size.x * 100f / playableAreaSize.x, _size.y * 100f / playableAreaSize.y);
		}
		...
		protected void AutoSize (GUIContent content)
		{
			...
			float xScale = ACScreen.safeArea.width;
			float maxWidth = xScale / 2f;
